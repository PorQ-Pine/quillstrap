...
impl SetupThing for Orbit {
    ...
    fn is_built(&self) -> bool {
        path_exists("target/aarch64-unknown-linux-gnu/release/orbit")
    }
    ...
    fn deploy(&self, _options: &crate::Options) -> color_eyre::eyre::Result<(), String> {
        ...
        ssh_execute("killall -9 orbit", port, _options);
        ssh_execute("rm -rf /usr/bin/orbit", port, _options);
        ssh_send(
            "target/aarch64-unknown-linux-gnu/release/orbit",
            "/usr/bin/orbit",
            port,
            _options,
        );
